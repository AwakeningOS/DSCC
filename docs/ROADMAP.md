...
M3 may advance in parallel with M1, while arbitrary remote execution remains gated by M2. GPU and WAN training are separate capabilities, not a single switch. Storage-only users are legitimate participants. M8 is described in [Distributed Open Model Commons](proposals/DISTRIBUTED_OPEN_MODEL_COMMONS.ja.md) and must reuse demonstrated decentralized-training work rather than re-inventing it. The complete product is not defined by this starter's small demo workload.
